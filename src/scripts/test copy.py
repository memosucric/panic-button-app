...
# Check if the LOGS_ENABLED environment variable is set to "true" or "yes" to enable logging
# logs_enabled = os.getenv("LOGS_ENABLED", "").lower() in ("true", "yes")
logs_enabled = os.getenv("LOGS_ENABLED", "true").lower() not in ("false", "no")

# Specify the log file path if logging is enabled
log_file_path = '/tmp/anvil.log' if logs_enabled else None

# Configure logging to write to the specified log file if enabled
if logs_enabled:
